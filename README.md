...
| `bot_name_map` | dict | 空 | 按群自定义昵称，面板「添加」：键=群号，值=昵称 |
| `bot_name_by_self_id` | dict | 空 | 按Bot账号自定义昵称（唤醒词），面板「添加」：键=Bot的QQ号，值=唤醒词 |
...
面板上点「添加」键值对：键填 `748791823`、值填 `老大`；再添加一组：键 `1092801060`、值 `宁宁`。

在群 748791823 中 @机器人 会替换为"老大"，在群 1092801060 中替换为"宁宁"，其他群用 `bot_name`。

### 按Bot账号昵称示例（多账号挂载）

面板上点「添加」键值对：键填 `10001`、值填 `宁宁`；再添加一组：键 `10002`、值填 `小助手`。
